...
applyrules(Client *c)
{
	...
	if(mon) {
		...
		c->geom.y = (mon->w.width - c->geom.width) / 2 + mon->m.y;
	}
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	if (corner_radius > 0) {
		c->round_border = wlr_scene_rect_create(c->scene, 0, 0, c->isurgent ? (float[])COLOR(colors[SchemeUrg][ColBorder]) : (float[])COLOR(colors[SchemeSel][ColBorder]));
	...
	if ((p = client_get_parent(c))) {
		...
		if(p->mon) {
			...
			c->geom.y = (p->mon->w.width - c->geom.width) / 2 + p->mon->m.y;
		}
